...
class BasicSwap(BaseApp, BSXNetwork, UIApp):
    ...
    def __init__(
        ...
        part_chain = self.settings.get("chainclients", {}).get("particl", {}).get("chain_override", self.chain)  # TODO NAV remove part_chain upon creating a pr
    ...
    def setCoinConnectParams(self, coin):
        ...
        for setting_name in (
            ...
            "electrum_port",
            "electrum_ssl",
            "chain_override",  # TODO NAV revert this upon making pr
        ):
    ...
    def createInterface(self, coin):
        if coin == Coins.PART:
            part_chain = self.coin_clients[coin].get("chain_override", self.chain)  # TODO NAV remove part_chain upon creating a pr
            ...
            if bid.initiate_tx is not None:
                ...
                if offer.lock_type < TxLockTypes.ABS_LOCK_BLOCKS:
                    ...
                    if coin_from == Coins.NAV:
                        script = ci_from.createFakeNonNavHTLCScript(secret_hash, sequence)
                    else:
                ...
                blinding_key = None
                if coin_from == Coins.NAV:
                    locktime = sequence if offer.lock_type < TxLockTypes.ABS_LOCK_BLOCKS else lock_value
                    txn, lock_tx_vout, nav_addr_redeem, nav_addr_refund, blinding_key = nav_logic.create_initiate_txn(self, bid_id, bid, offer, ci_from, locktime, secret_hash, bid_date, use_cursor)
                else:
                ...
                if coin_from == Coins.NAV:
                    locktime = sequence if offer.lock_type < TxLockTypes.ABS_LOCK_BLOCKS else lock_value
                    nav_logic.import_itx_and_send_payload_msg_to_bidder(self, bid_id, bid, offer, ci_from, nav_addr_redeem, nav_addr_refund, secret_hash, locktime, blinding_key, txn_funded, chain_height_before_submit, use_cursor)
            ...
            if txid is not None:
                ...
                if Coins.NAV in (coin_from, coin_to):
                    ...
                    if coin_from == Coins.NAV:
                        ...
                        # Send ptx redeem address to the bidder
    ...
    def deriveParticipateScript(self, bid_id: bytes, bid, offer) -> bytearray:
        ...
        if offer.lock_type < TxLockTypes.ABS_LOCK_BLOCKS:
            ...
            nav_locktime = sequence
            if coin_to == Coins.NAV:
                participate_script = ci_to.createFakeNonNavHTLCScript(secret_hash, sequence)
            else:
            ...
            nav_locktime = contract_lock_value
            if coin_to == Coins.NAV:
        ...
        return participate_script, nav_locktime

    def createParticipateTxn(
        self, bid_id: bytes, bid, offer, participate_script: bytearray, nav_locktime: int
    ):
        ...
        if coin_to == Coins.NAV:
            return nav_logic.create_nav_ptx(self, bid_id, bid, offer, ci, nav_locktime, participate_script)
    ...
    def createRedeemTxn(
        ...
        if coin_type == Coins.NAV:
            ...
            prevout = nav_logic.build_nav_redeem_prevout(self, bid, ci, nav_txn, privkey, txn_script, is_ptx)
        else:
            prevout = {
                "txid": prev_txnid,
                "vout": prev_n,
    ...
    def createRefundTxn(
        ...
        if coin_type in (Coins.DCR,):
            ...
            prevout = nav_logic.build_nav_refund_prevout(self, bid, ci, txn, secret_hash, addr_refund_out)
        else:
    ...
    def initiateTxnConfirmed(self, bid_id: bytes, bid, offer) -> None:
        ...
        participate_script, nav_locktime = self.deriveParticipateScript(bid_id, bid, offer)
        if bid.was_sent:
            if bid.participate_tx is not None:
                ...
                ci_to = self.ci(offer.coin_to)
                txn, nav_ptx_import_payload = self.createParticipateTxn(bid_id, bid, offer, participate_script, nav_locktime)
                if Coins(offer.coin_to) == Coins.NAV:
    ...
    def checkBidState(self, bid_id: bytes, bid, offer):
        ...
        if state == BidStates.BID_ACCEPTED:
            ...
            if coin_from == Coins.PART:  # Has txindex
                ...
                if coin_from == Coins.NAV:
                    found = nav_logic.is_initiate_txn_on_chain(self, bid_id, bid, ci_from)
                else:
                ...
                if found:
                    ...
                    if coin_from == Coins.NAV:
                        ...
                        if outid:
                            ...
                            save_bid = True
                elif coin_from == Coins.NAV and nav_logic.is_nav_itx_refunded(self, bid_id, bid, ci_from):
                    return True
            ...
            if bid.participate_tx.conf is not None:
                ...
                if (
                    ...
                    save_bid = True
            if coin_from == Coins.NAV:
                if nav_logic.detect_nav_itx_refund(self, bid_id, bid, self.ci(coin_from)):
                    save_bid = True
