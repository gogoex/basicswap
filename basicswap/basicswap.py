...
class BasicSwap(BaseApp, BSXNetwork, UIApp):
    ...
    def getWalletKey(self, coin_type, key_num, for_ed25519=False, for_bls=False) -> bytes:
        ...
        if for_bls:
            return self.deriveBLSKey(coin_type, evkey, key_path_base)
        elif for_ed25519:
    ...
    def getLockValue(self, ci_from, offer) -> int:  
        ...
        if offer.lock_type == TxLockTypes.SEQUENCE_LOCK_TIME and ci_from.coin_type() == Coins.NAV:
            ...
            self.log.info("getLockValue lock_type is NOT ABS_LOCK_BLOCKS, {lock_value=}")
        self.log.info(
    ...
    def acceptBid(self, bid_id: bytes, cursor=None) -> None:
        ...
        try:
            ...
            self.log.info(f"---> offer={offer.__dict__}, bid={bid.__dict__}")
            ...
            self.log.info(f"---> bid is still valid") 
            ...
            self.log.info(f"---> compatible bid protocol version") 
            ...
            if bid.initiate_tx is not None:
                self.log.info("---> initiate tx already exists")
                ...
                if coin_from == Coins.NAV:
                    ...
                    self.log.info(f"---> 1 sequence is {sequence}")
                    ...
                    self.log.info(f"---> 1 built contract script: {script}")
                else:
                    lock_value = self.getLockValue(ci_from, offer)
                    self.log.info("---> 2 building contract script")
                    script = atomic_swap_1.buildContractScript(
                        lock_value,
                        secret_hash,
                        bid.pkhash_buyer,
                    ...
                    self.log.info("---> 2 built contract script: {script}")

                bid.pkhash_seller = ci_to.pkh(pubkey_refund)
                self.log.info(f"---> seller pkhash is {bid.pkhash_seller.hex()}")
                ...
                if coin_from == Coins.NAV:
                    # Import HTLC script so wallet tracks the output after
                    # tx aggregation (txid changes when mined).
                    params = {
                        "type": "atomic_swap",
            ...
            if txid is not None:
                ...
                self.log.info(f"---> prepared BID_ACCEPT message")
    ...
    def createInitiateTxn(
        ...
        self.log.info(f"---> Create {ci.coin_name()} initiate txn w/ to: {addr_to}")
        ...
        self.log.info(f"---> decoded sign tx is: {txjs}")
    ...
    def deriveParticipateScript(self, bid_id: bytes, bid, offer) -> bytearray:
        ...
        self.log.debug(f"---> {lock_value=}, offer.lock_value={offer.lock_value}")

        self.log.debug(f"---> secret_hash: {secret_hash.hex()} ({len(secret_hash)}), pkhash_seller: {pkhash_seller.hex()} ({len(pkhash_seller)})")
    ...
    def createParticipateTxn(
        ...
        self.log.debug("---> createParticipateTxn called")
    ...
    def createRedeemTxn(
        ...
        if coin_type == Coins.NAV:
            # select if-path and push secret to the stack
        ...
        if self.debug:
            ...
            if ci.get_connection_type() == "rpc":
                ...
                if coin_type == Coins.NAV:
                    prev_outid = redeem_txjs["vin"][0]["outid"]
                    redeem_txid = redeem_txjs["txid"]
                    self.log.debug(
                        f"Have valid redeem tx {self.log.id(redeem_txid)} for contract {for_txn_type} tx {self.log.id(prev_outid)}"
                    )
    ...
    def createRefundTxn(
        ...
        if coin_type == Coins.NAV:
            amount_out = prevout["amount"] * 1e-8 - tx_fee # prevout["amount"] is in navoshis
        else: 
            amount_out = ci.make_int(prevout["amount"], r=1) - ci.make_int(tx_fee, r=1)
        if amount_out <= 0:
            raise ValueError(f"Refund amount out ({amount_out}) <= 0")
    ...
    def initiateTxnConfirmed(self, bid_id: bytes, bid, offer) -> None:
        ...
        self.log.info(f"---> created participate script")
        if bid.was_sent:
    ...
    def processNavPtxImport(self, msg) -> None:
        ...
        self.log.debug(f"processNavPtxImport from {msg['from']}")
        msg_bytes = self.getSmsgMsgBytes(msg)
        offset = 0
    ...
    def checkBidState(self, bid_id: bytes, bid, offer):
        ...
        if state == BidStates.BID_ACCEPTED:
            self.log.debug(f"---> state == BID_ACCEPTED: {coin_from=}, {coin_to=}")
            ...
            if bid.initiate_tx.conf is not None:
                ...
                self.log.debug(
                    f"{coin_from=} blocks_confirmed: {self.coin_clients[coin_from]['blocks_confirmed']}"
                )
                if (
