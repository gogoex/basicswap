...
NAVIO_VERSION = os.getenv("NAVIO_VERSION", "0.1.3")
...
expected_key_ids = {
    ...
    "navio_builder": ("5E542D3BDB5F4305762330B37A17AA0838680153",),
    "nicolasdorier": (
...
def tryPrepareCore(coin, version_data, signer, settings, data_dir, extra_opts={}):
    ...
    if coin == "monero":
        ...
        if coin == "particl":
            ...
            release_tag = "v" + version + version_tag
            release_filename = f"{coin}-{version}{version_tag}-{bin_arch}.{FILE_EXT}"
    ...
    if coin == "navio":
        pubkeyurls.append("https://nav.io/releases.asc")
